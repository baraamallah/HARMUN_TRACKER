    ...
    <div className="min-h-screen flex flex-col bg-background">
      <header className="sticky top-0 z-10 border-b bg-background/80 backdrop-blur-sm">
        <div className="container mx-auto h-16 flex items-center justify-between px-4 md:px-6">
          {isLoadingLogo ? <Skeleton className="h-10 w-36" /> : <Logo customLogoUrl={munLogoUrl} />}
          <ThemeToggleButton />
        </div>
      </header>
      ...
      <footer className="py-6 border-t">
        <div className="container mx-auto px-4 md:px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          {isLoadingLogo ? <Skeleton className="h-8 w-32" /> : <Logo customLogoUrl={munLogoUrl} variant="circled-icon" size="sm" />}
