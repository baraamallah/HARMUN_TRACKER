...
import { PlusCircle, Loader2, Users2 as UsersIcon, Layers, Trash2, Filter, Calendar, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-6 pb-4">
      ...
      {!isLoading && filteredStaffMembers.length > 0 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-2 py-4 border-t bg-muted/20 rounded-xl">
          <div className="flex items-center gap-2 order-2 sm:order-1">
            <span className="text-xs sm:text-sm text-muted-foreground font-medium">
              Showing <span className="text-foreground font-bold">{((currentPage - 1) * pageSize) + 1}</span> to <span className="text-foreground font-bold">{Math.min(currentPage * pageSize, filteredStaffMembers.length)}</span> of <span className="text-foreground font-bold">{filteredStaffMembers.length}</span>
            </span>
          </div>
